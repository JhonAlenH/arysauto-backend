...
router.route('/provider-excluded').post((req, res) => {
    if(!req.header('Authorization')){ 
        res.status(400).json({ data: { status: false, code: 400, message: 'Required authorization header not found.' } })
        return;
    }else{
        operationValrepProviderExcluded(req.header('Authorization'), req.body).then((result) => {
            if(!result.status){ 
                res.status(result.code).json({ data: result });
                return;
            }
            ...
            res.status(500).json({ data: { status: false, code: 500, message: err.message, hint: 'operationValrepProviderExcluded' } });
        });
    }
});

const operationValrepProviderExcluded = async(authHeader, requestBody) => {
    if(!helper.validateAuthorizationToken(authHeader)){ return { status: false, code: 401, condition: 'token-expired', expired: true }; }
    //if(!helper.validateRequestObj(requestBody, ['cproveedor'])){ return { status: false, code: 400, message: 'Required params not found.' }; }
    let query = await bd.providerExcludedValrepQuery().then((res) => res);
    if(query.error){ return { status: false, code: 500, message: query.error }; }
    let jsonArray = [];
    for(let i = 0; i < query.result.recordset.length; i++){
...
const operationValrepCharge = async(authHeader, requestBody) => {
    ...
    for(let i = 0; i < query.result.recordset.length; i++){
        jsonArray.push({ 
            xcliente: query.result.recordset[i].XCLIENTE, 
            xpoliza: query.result.recordset[i].XPOLIZA, 
...
router.route('/status').post((req, res) => {
    if(!req.header('Authorization')){ 
        ...
        operationValrepStatus(req.header('Authorization'), req.body).then((result) => {
            if(!result.status){ 
                res.status(result.code).json({ data: result });
                return;
            }
            ...
            res.status(500).json({ data: { status: false, code: 500, message: err.message, hint: 'operationValrepStatus' } });
        });
    }
});

const operationValrepStatus = async(authHeader, requestBody) => {
    if(!helper.validateAuthorizationToken(authHeader)){ return { status: false, code: 401, condition: 'token-expired', expired: true }; }
    let query = await bd.statusValrepQuery().then((res) => res);
    if(query.error){ return { status: false, code: 500, message: query.error }; }
    let jsonArray = [];
    for(let i = 0; i < query.result.recordset.length; i++){
        jsonArray.push({ cestatusgeneral: query.result.recordset[i].CESTATUSGENERAL, xestatusgeneral: query.result.recordset[i].XESTATUSGENERAL });
